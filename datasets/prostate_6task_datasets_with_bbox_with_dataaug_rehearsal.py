# 2024-09-29 16:32
# change from ./vessel_9cls_3task_datasets_with_bbox_with_dataaug_rehearsal.py
# 1. order 变量更新，这个数据集有6个任务，需要给出全排列
# 2. 更新 get_mask 函数，item["points"] 是一个路径，直接从这个路径读取 mask 即可
# 3. 更新 json 文件名: 在每个 dataset 的 load_dataset 函数中，将 json 文件名更新为 "prostate_image_dataset_task{idx+1}.json"  
# 4. 更新 随机边框外扩的取值范围：100 -> 20
# 5. 更新 JointDataset __init__ 函数中的 range(3) -> range(6)，并更新 json 文件名
# 6. 更新 VesselSeqDataset load_dataset 函数中的 _shuffle.json 文件
    ...
    def load_dataset(self, task_id):
        ...
        if task_id > 0:
            for i in range(task_id):
                ...
                with open(os.path.join(self.data_root, f"prostate_image_dataset_task{i_+1}_shuffle.json"), 'r') as f:
