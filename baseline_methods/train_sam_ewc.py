...
import cv2
import json
# from datasets.vessel_9cls_3task_datasets_with_bbox_with_dataaug import VesselSeqDataset, VesselTestDataset, VesselJointDataset
...
parser.add_argument("-task_name", type=str, default="SAM-EWC")
...
def update_dict(fisher, params, model, train_loader, seg_loss, ce_loss):
    fisher_, params_ = {}, {}
    ...
    for step, (image, gt2D, boxes, _, cropped_image) in enumerate(tqdm(train_loader)):
        boxes_np = boxes.detach().cpu().numpy()
        image, gt2D = image.to(device), gt2D.to(device)
        medsam_pred = model(image, boxes_np, train=True)
        loss = seg_loss(medsam_pred, gt2D) + ce_loss(medsam_pred, gt2D.float())
        optimizer.zero_grad()
        loss.backward()
        for name, param in model.named_parameters():
            if param.requires_grad and param.grad is not None:
                assert "prompt_encoder" not in name
                if name not in fisher_:
                    fisher_[name] = param.grad.clone() ** 2
                else:
                    fisher_[name] += param.grad.clone() ** 2
    
    for name, param in model.named_parameters():
        if param.requires_grad and param.grad is not None:
            params_[name] = param.data.clone()
            fisher_[name] = fisher_[name] / len(train_loader)
    
    fisher.append(fisher_)
    params.append(params_)

    return fisher, params

def ewc_loss(fisher, params, model, ewc_lambda):
    loss = 0
    for fisher_, params_ in zip(fisher, params):
        for name, param in model.named_parameters():
            if param.requires_grad and (name in params_.keys()):
                loss = loss + (fisher_[name] * (param - params_[name]) ** 2).sum() * ewc_lambda / 2
    return loss
#########################################################################
...
def main():
    ...
    for task in range(args.task_num):
        ...
        train_dataloader = DataLoader(
            train_dataset,
            batch_size=args.batch_size if task <= 3 else 1,
            shuffle=True,
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for step, (image, gt2D, boxes, _, cropped_image) in enumerate(tqdm(train_dataloader)):
                ...
                if args.use_amp:
                    ...
                    loss = seg_loss(medsam_pred, gt2D) + ce_loss(medsam_pred, gt2D.float())
                    ################################## ewc #################################
                    loss = loss + ewc_loss(fisher, params, medsam_model, args.ewc_lambda)
                    ########################################################################
                    loss.backward()
                    optimizer.step()
                    optimizer.zero_grad()
        ...
        # plt.close()
        ################################### ewc ########################################
        fisher, params = update_dict(fisher, params, medsam_model, train_dataloader, seg_loss, ce_loss)
        ################################################################################
